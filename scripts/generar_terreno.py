...
def generar_obj_terreno(filename="lunar_terrain.obj", grid_size=200):
    size_px = grid_size
    ...
    terreno_suavizado = cv2.GaussianBlur(terreno, (31, 31), 0)

    # Convertir a metros para calcular las normales reales
    Z = (terreno_suavizado / 255.0) * 0.5
    dx = 10.0 / (size_px - 1)
    dy = 10.0 / (size_px - 1)

    print("Calculando normales para la iluminación...")
    # Calcular pendientes (gradientes) en X y Y
    ...
    print("Escribiendo malla 3D, por favor espera un momento...")
    with open(filename, 'w') as f:
        f.write("# Terreno Lunar M4F00CV con Normales\n")
        
        # Vértices (v x y z) y Normales (vn x y z)
        ...
        # Caras indexando Vértice y Normal (f v1//vn1 v2//vn2 v3//vn3)
...
if __name__ == '__main__':
    ...
    generar_obj_terreno(ruta_salida)
